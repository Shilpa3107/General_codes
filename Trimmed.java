...
public class Trimmed{
    public static void main(String args[]){
         ...
         int arr[] = trimmed(nums,queries);
         for(int n : arr)
           System.out.print(n+" ");
    }
    public static int[] trimmed(String nums[], int queries[][]){
        ...
        for(int i = 0;i<queries.length;i++){
            ...
            for(int j = 0;j<nums.length;j++){
                pq.offer(new String[]{Integer.toString(j),nums[j].substring(nums[j].length()-trim)});
            }
            while(k-->0)
              pq.poll();
